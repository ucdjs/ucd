...
Every hook context includes `phase: "start" | "end"`. Hooks may be async and are intended for side effects. Throwing from a hook fails the current execution path.
