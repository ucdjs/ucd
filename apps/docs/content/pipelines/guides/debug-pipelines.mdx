...
The examples below use the public `ucd` CLI shape. If you are working inside the UCD.js monorepo, you can run the same commands through the workspace binary instead:

```bash
./packages/cli/bin/ucd.js pipelines ...
```
...
```bash
ucd pipelines list --cwd ./my-pipelines
```

Useful variations:

```bash
ucd pipelines list --cwd ./my-pipelines --path src/simple
```
...
```bash
ucd pipelines run my-pipeline --cwd ./my-pipelines
```
...
```bash
ucd pipelines run --ui --cwd ./my-pipelines
```

Or choose a specific port:

```bash
ucd pipelines run --ui --port 4040 --cwd ./my-pipelines
```
