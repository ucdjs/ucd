...
description: Detailed package dependency graph organized by layer
---
...
| `@ucdjs/fs-bridge` | Filesystem abstraction — swap between Node.js disk I/O and HTTP read access |
...
```mermaid
graph LR
  APP["Application"] --> FB["fs-bridge interface\nread / write / listdir / stat / rm"]
  FB --> N["NodeFSBridge\n(local disk)"]
  FB --> H["HTTPFSBridge\n(read-only HTTP)"]
```

The bridge abstraction means storage code is environment-agnostic. The same `lockfile` and `ucd-store` code runs against a local filesystem in development and against a remote HTTP endpoint in read-only contexts.
...
The lockfile is the canonical record of what has been mirrored. It is the only mutable artifact that both the write path (pipelines/sync) and the read path (client/API) agree on.

## Layer 4 — Processing (Pipelines)

| Package | Purpose |
|---|---|
| `@ucdjs/pipelines-core` | Pipeline DSL: `definePipeline()`, sources, routes, transforms, filters, events |
| `@ucdjs/pipelines-graph` | DAG representation of pipeline route dependencies |
| `@ucdjs/pipelines-artifacts` | Artifact validation and lifecycle |
| `@ucdjs/pipelines-loader` | Discover and load pipeline definitions from the filesystem |
| `@ucdjs/pipelines-executor` | Execute pipelines with concurrency, caching, and event emission |
| `@ucdjs/pipelines-presets` | Built-in pipeline definitions for common UCD processing tasks |
