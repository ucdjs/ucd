...
- `run()` executes one pipeline with version-isolated work, route DAG ordering, and deterministic output merging
...
sequenceDiagram
  ...
  Run->>Run: merge version outputs in declared version order
...
sequenceDiagram
  ...
  alt cached result hit
    ...
    Match-->>Runtime: buffered route outputs
  end
  Runtime->>Outputs: materialize merged output definitions
```
...
- Versions execute in isolated contexts and their outputs are merged in declared version order.
