...
try {
  ...
  if (err instanceof PathUtilsBaseError) {
    ...
    console.error('Path utility error:', err.message);
  }
}
```
...
try {
  ...
  if (error instanceof PathTraversalError) {
    ...
    console.error(`Base: ${err.basePath}`);
    console.error(`Attempted: ${err.accessedPath}`);
    // Base: /var/www/html
...
try {
  ...
  if (err instanceof WindowsDriveMismatchError) {
    console.error(`Drive mismatch!`);
    console.error(`Base drive: ${err.baseDrive}`);
    console.error(`Attempted drive: ${err.accessedDrive}`);
    // Base drive: C
    // Attempted drive: D
  }
}
...
try {
  ...
  if (err instanceof UNCPathNotSupportedError) {
    console.error(`UNC path rejected: ${err.path}`);
...
try {
  ...
  if (err instanceof IllegalCharacterInPathError) {
    console.error('Illegal character detected:', err.message);
...
try {
  ...
  if (err instanceof FailedToDecodePathError) {
...
try {
  ...
  if (err instanceof MaximumDecodingIterationsExceededError) {
...
try {
  ...
  if (err instanceof WindowsPathBehaviorNotImplementedError) {
...
function handlePath(basePath: string, userInput: string): string | null {
  try {
    ...
    if (err instanceof PathUtilsBaseError) {
      // Log security event
      console.warn('Path security violation:', err.message);
      return null;
    }
    throw err; // Non path-utils errors
  }
}
```
...
try {
  ...
  if (err instanceof PathTraversalError) {
    logSecurityEvent(err, userInput, currentUserId);
