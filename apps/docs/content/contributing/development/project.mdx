...
The API (apps/api) is a Cloudflare Worker using Hono with OpenAPI. It serves versioned routes for files, reports, versions, and schemas, and can fetch data directly from upstream sources.
...
- The API file surface uses public Unicode versions in request paths. For most versions this matches the upstream Unicode.org layout directly, while legacy versions with a different UCD directory name are resolved internally.
