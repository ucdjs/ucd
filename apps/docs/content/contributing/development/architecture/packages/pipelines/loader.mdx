...
```mermaid
flowchart TD
  A["loadPipelinesFromPaths()"] --> B["loadPipelineFile() for each file"]
  B --> C["Return LoadedPipelineFile { pipelines, exportNames, issues }"]
  C --> D["Flatten pipelines and issues"]
  D --> E["Return { pipelines, files, issues }"]
```
...
- `loadPipelineFile()` reports expected bundle/import/validation problems through `issues` instead of throwing. Consumers should always check `result.issues`.
