...
UCD.js uses [Vitest](https://vitest.dev/) for testing.
...
- **HTTP Mocking**: Use `mockFetch` from `@ucdjs/test-utils/msw` for HTTP/MSW-driven tests.
- **API Mocking**: Prefer `mockStoreApi` from `@ucdjs/test-utils/mock-store` so tests stay consistent with the API surface.
- **Filesystem Tests**: Use `testdir()` from `vitest-testdirs` to get a temporary directory that is cleaned up automatically.
- **Shared Helpers**: Rely on `@ucdjs/test-utils/*` for helpers that need to be shared across packages.
...
<Cards>
  <Card title="Project Architecture" href="/contributing/development/project" description="Learn about the monorepo structure and core systems." />
</Cards>
