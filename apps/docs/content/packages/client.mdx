...
flowchart TD
  ...
  C --> D["Create files + reports + versions resources from config.endpoints"]

  E["createUCDClientWithConfig(baseUrl, config)"] --> D

  D --> F["client.files.get(path)"]
  D --> G["client.reports.list() / get(reportId)"]
  D --> H["client.versions.list() / getFileTree(version)"]

  I["client.config.get()"] --> J["GET /.well-known/ucd-config.json"]
  K["client.versions.getManifest(version)"] --> L["GET config.endpoints.manifest"]
```
...
<TypeTable
  type={{
    ...
    reports: {
      description: 'List Unicode reports and fetch revision metadata or HTML',
...
<Tabs items={['files', 'reports', 'versions', 'config']}>
  <Tab value="files">
    ```typescript
