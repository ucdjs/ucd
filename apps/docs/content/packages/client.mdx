...
The `@ucdjs/client` package provides a type-safe API client for interacting with the UCD.js API. It provides resource-based helpers for Unicode data files, versions, version manifests, and published server configuration.
...
flowchart TD
  ...
  J["client.versions.getManifest(version)"] --> K["GET config.endpoints.manifest"]
```
...
<TypeTable
  type={{
    ...
    manifest: {
      description: 'Deprecated alias for `client.versions.getManifest(version)`',
...
<Tabs items={['files', 'versions', 'config', 'manifest']}>
  ...
  <Tab value="manifest">
    ...
    console.log("Expected files for 16.0.0:", manifest.expectedFiles);
    ```
  </Tab>
</Tabs>
