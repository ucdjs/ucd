...
- [`PathTraversalError`](/packages/path-utils/errors#pathtraversalerror) - If the resolved path would escape the base directory
- [`WindowsDriveMismatchError`](/packages/path-utils/errors#windowsdrivemismatcherror) - If attempting to access a different drive letter on Windows
- [`UNCPathNotSupportedError`](/packages/path-utils/errors#uncpathnotsupportederror) - If a UNC path is provided
- [`IllegalCharacterInPathError`](/packages/path-utils/errors#illegalcharacterinpatherror) - If the path contains null bytes or control characters
- [`FailedToDecodePathError`](/packages/path-utils/errors#failedtodecodepatherror) - If the path cannot be safely decoded
- [`WindowsPathBehaviorNotImplementedError`](/packages/path-utils/errors#windowspathbehaviornotimplementederror) - If an unsupported Windows path combination is encountered
- `TypeError` - If `basePath` is not a string
- `Error` - If `basePath` is an empty string
...
- [`MaximumDecodingIterationsExceededError`](/packages/path-utils/errors#maximumdecodingiterationsexceedederror) - If decoding exceeds 10 iterations (malicious input protection)
