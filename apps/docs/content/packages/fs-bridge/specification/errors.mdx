...
<Callout type="warn">
  This page documents deprecated `@ucdjs/fs-bridge` errors. For new integrations, prefer the <a href="/packages/fs-backend/specification/errors">FS Backend error model</a> and use the <a href="/packages/fs-backend/migration-from-fs-bridge">migration guide</a> when updating existing consumers.
</Callout>
...
try {
  ...
  if (err instanceof BridgeGenericError) {
...
try {
  ...
  if (err instanceof BridgeSetupError) {
    console.error("Failed to setup bridge:", err.message);
    console.error("Original error:", err.originalError);
  }
}
```
...
try {
  ...
  if (err instanceof BridgeUnsupportedOperation) {
    console.error(`Bridge doesn't support ${err.capability} operation`);
  }
}
```
...
try {
  ...
  if (err instanceof BridgeEntryIsDir) {
    console.error(`Expected file but found directory: ${err.path}`);
  }
}
```
...
try {
  ...
  if (err instanceof BridgeFileNotFound) {
    console.error("File not found:", err.path);
  } else if (err instanceof BridgeUnsupportedOperation) {
    console.error("Operation not supported:", err.capability);
  } else {
