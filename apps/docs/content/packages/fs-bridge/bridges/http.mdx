...
<Callout type="warn">
  This bridge is part of deprecated `@ucdjs/fs-bridge`. Prefer the <a href="/packages/fs-backend/bridges/http">FS Backend HTTP backend</a> for new code, and use the <a href="/packages/fs-backend/migration-from-fs-bridge">migration guide</a> when switching existing consumers.
</Callout>
...
try {
  ...
  if (err.message.includes("Failed to read remote file")) {
    // Handle read error
  }
}
...
try {
  ...
  if (err.message.includes("Server error")) {
    // Handle server error
  } else if (err.message.includes("Invalid response schema")) {
...
The bridge supports all bridge hooks for monitoring and debugging operations. See the [Hooks documentation](/packages/fs-bridge/hooks) for complete details and examples.
