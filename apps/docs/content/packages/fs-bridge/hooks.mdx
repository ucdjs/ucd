...
<Callout type="warn">
  `@ucdjs/fs-bridge` is deprecated. For new integrations, prefer <a href="/packages/fs-backend"><code>@ucdjs/fs-backend</code></a> and its <a href="/packages/fs-backend/hooks">hooks documentation</a>. Use the <a href="/packages/fs-backend/migration-from-fs-bridge">migration guide</a> when updating existing bridge consumers.
</Callout>
