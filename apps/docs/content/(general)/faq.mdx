...
Both! The HTTP client and backend-compatible storage tooling are designed to work across environments. You can fetch Unicode data from HTTP in the browser or work with local files in Node.js/Bun.
...
Absolutely. The `@ucdjs/pipeline-*` system is incredibly robust and pluggable. You can define custom `Sources` and `Routes` to parse whatever specific format you need while leveraging the built-in graph execution.
