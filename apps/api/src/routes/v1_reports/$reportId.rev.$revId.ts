...
import { getUnicodeReportHtml, getUnicodeReportRevisionMetadata, isValidReportId } from "#lib/reports";
...
const GET_REPORT_HTML_ROUTE = createRoute({
  ...
  description: dedent`
    Fetch the HTML document for a specific Unicode report revision.
...
export function registerGetReportHtmlRoute(router: OpenAPIHono<HonoEnv>) {
  router.openapi(GET_REPORT_HTML_ROUTE, async (c) => {
    ...
    const { body, headers, status } = await getUnicodeReportHtml(reportId, revId);
    if (status !== 200) {
