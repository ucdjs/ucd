...
Bridges automatically detect which optional operations they support. The `optionalCapabilities` property is a map indicating which optional operations are available:
...
The bridge system defines several error types:
...
Generic error wrapper that includes the original error.
...
Thrown when bridge setup fails.
...
Thrown when a file operation is attempted on a directory.
