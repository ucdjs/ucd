...
export async function generateFields(options: GenerateFieldsOptions): Promise<string | null> {
  ...
  try {
    const result = await generateObject({
      ...
      prompt: dedent`
        You are an expert code generator assistant specializing in creating well-documented TypeScript type definitions and data structures. Given a text description, your objective is to:
        ...
        Your output must be a single TypeScript code block that is syntactically correct, adheres to best practices, and includes **detailed JSDoc comments** for the interface, its properties, and any nested types or the keys array. Ensure strict snake_case naming for all field name and properties within the interface, guarantee type safety (using union types instead of enums), and meticulously maintain the order.
