...
export function assertCapability<T extends OptionalCapabilityKey = never>(
  ...
  for (const capability of capabilitiesToCheck) {
    if (!bridge.optionalCapabilities[capability]) {
      debug?.("Bridge capability check failed", { capability, availableCapabilities: Object.keys(bridge.optionalCapabilities).filter((k) => bridge.optionalCapabilities[k as OptionalCapabilityKey]) });
