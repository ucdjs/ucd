...
describe("ChainTransforms (types)", () => {
  it("infers the final output type", () => {
    type T1 = PipelineTransformDefinition<string, number>;
    type T2 = PipelineTransformDefinition<number, boolean>;
    type Result = ChainTransforms<string, [T1, T2]>;

    expectTypeOf<Result>().toEqualTypeOf<boolean>();
  });

  it("returns the input type for an empty chain", () => {
    type Result = ChainTransforms<string, []>;

    expectTypeOf<Result>().toEqualTypeOf<string>();
  });

  it("falls back to input type when the first transform mismatches", () => {
    type T1 = PipelineTransformDefinition<number, string>;
    type Result = ChainTransforms<string, [T1]>;
