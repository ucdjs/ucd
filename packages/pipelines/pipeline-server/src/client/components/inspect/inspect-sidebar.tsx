...
export function InspectSidebar() {
  ...
  const filteredRoutes = useMemo(() => {
    ...
    return pipeline.routes.filter((route) => {
      ...
      if (route.transforms.some((transform) => transform.toLowerCase().includes(normalizedFilter))) return true;
      if (route.outputs.some((output) =>
        (output.dir ?? "").toLowerCase().includes(normalizedFilter)
        || (output.fileName ?? "").toLowerCase().includes(normalizedFilter),
      )) return true;

      return route.emits.some((emit) => emit.id.toLowerCase().includes(normalizedFilter))
        || route.depends.some((dependency) =>
          dependency.type === "route"
            ? dependency.routeId.toLowerCase().includes(normalizedFilter)
            : `${dependency.routeId}:${dependency.artifactName}`.toLowerCase().includes(normalizedFilter),
        );
    });
  }, [normalizedFilter, pipeline.routes]);

  const filteredTransforms = useMemo(() => {
    if (!normalizedFilter) return transforms;
    return transforms.filter((transform) => transform.name.toLowerCase().includes(normalizedFilter));
  }, [normalizedFilter, transforms]);
  ...
  const filteredOutputs = useMemo(() => {
    ...
    return outputs.filter((output) =>
      output.dir.toLowerCase().includes(normalizedFilter)
      || output.fileName.toLowerCase().includes(normalizedFilter)
      || output.routeId.toLowerCase().includes(normalizedFilter),
    );
  }, [normalizedFilter, outputs]);
