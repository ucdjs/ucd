          ...
          {levels.map((level) => {
            ...
            return (
              <div key={level} className="relative h-8 flex items-center">
                ...
                {/* Event bars with adjusted positioning */}
                {levelEvents.map((event, index) => {
                  const isSelected = event.id === selectedEventId;
                  const color = getEventColor(event.type);

                  // Determine if event has duration
                  ...
                  // Calculate base position from timestamp
                  ...
                  if (totalDuration < 0.1) {
                    // Very fast execution - evenly distribute
                    leftPercent = (index / Math.max(levelEvents.length - 1, 1)) * 90;
                    widthPercent = hasDuration ? 8 : 6;
                  } else if (hasDuration) {
                    // Events WITH duration (like pipeline:end, version:end)
                    // These represent completed phases - bar extends BACKWARD from end time
                    const endPercent = (eventOffset / totalDuration) * 100;
                    const durationPercent = (event.durationMs / totalDuration) * 100;
                    
                    // Position bar so it ends at the event timestamp
                    // and extends backward by the duration
                    leftPercent = Math.max(0, endPercent - durationPercent);
                    widthPercent = Math.min(durationPercent, endPercent);
                  } else {
                    // Events WITHOUT duration (like pipeline:start)
                    // These are instant markers - make them wider so they're visible
                    leftPercent = (eventOffset / totalDuration) * 100;
                    widthPercent = 6; // Increased from 4 to make start events more visible
                  }
                  ...
                  return (
                    <button
                      ...
                      className={cn(
                        ...
                        color,
                        // Start events: sharp left edge, rounded right
                        // End events: rounded both sides
