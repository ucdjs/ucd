...
describe("Error inheritance and type checking", () => {
  it("should maintain correct inheritance chain and have unique names", () => {
    const baseError = new UCDStoreError("base");
    const fileError = new UCDStoreFileNotFoundError("/path", "1.0.0");
    const versionError = new UCDStoreVersionNotFoundError("1.0.0");
    const featureError = new UCDStoreUnsupportedFeature("feature", ["req"], ["avail"]);

    const errors = [baseError, fileError, versionError, featureError];

    errors.forEach((error) => {
      expect(error instanceof Error).toBe(true);
      expect(error instanceof UCDStoreError).toBe(true);
    });

    expect(fileError instanceof UCDStoreFileNotFoundError).toBe(true);
    expect(versionError instanceof UCDStoreVersionNotFoundError).toBe(true);
    expect(featureError instanceof UCDStoreUnsupportedFeature).toBe(true);

    const names = [baseError.name, fileError.name, versionError.name, featureError.name];
