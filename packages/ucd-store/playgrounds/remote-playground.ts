...
console.log("[remote-store] hasVersion: 15.0.0", await store.hasVersion("15.0.0"));
console.log("[remote-store] hasVersion: 99.0.0", await store.hasVersion("99.0.0"));
...
assert(store.fileCache.size > 0, "file cache should not be empty");
assert(store.fileCache.has("16.0.0/UnicodeData.txt"), "file cache should contain the file for 16.0.0/UnicodeData.txt");

console.log("[remote-store]", store);
