...
import { createPathFilter, filterTreeStructure, flattenFilePaths, PRECONFIGURED_FILTERS } from "@ucdjs-internal/shared";
import { createUCDClient } from "@ucdjs/client";
import { bundleCodegen, flattenVersion } from "@ucdjs/codegen";
...
async function collectRemoteFiles(
  ...
  for (const version of resolvedVersions) {
    ...
    const pathFilter = createPathFilter({
      include: ["**/*.txt"],
      exclude: [...PRECONFIGURED_FILTERS.TEST_FILES, ...PRECONFIGURED_FILTERS.README_FILES],
    });
    const filteredTree = filterTreeStructure(pathFilter, tree);
    const txtPaths = flattenFilePaths(filteredTree);
    ...
    for (const treePath of txtPaths) {
      ...
      output.info(`Fetching ${cleanPath}...`);
      ...
      output.success(`Fetched ${cleanPath}.`);
      files.push({
        content,
        filePath: cleanPath.startsWith(`${version}/`) ? cleanPath.slice(version.length + 1) : cleanPath,
        version,
      });
    }
  }
...
async function writeBundledFile(
  ...
  const bundledCode = bundleCodegen({ results, version });

  const bundleFileName = bundleTemplate.replace("{version}", flattenVersion(version));
  let bundlePath;
...
export async function runFieldCodegen({ versions, flags }: CLICodegenFieldsCmdOptions) {
  ...
  if (!shouldBundle) {
    await Promise.all(results.map(async (result) => {
      ...
      const outPath = path.join(outputDir, result.filePath.replace(/\.txt$/, ".ts"));
